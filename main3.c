...
SystemInit();
RCC->APB2ENR = RCC_APB2ENR_IOPAEN|RCC_APB2ENR_AFIOEN|RCC_APB2ENR_IOPBEN;  // Enable clock for GPIOB
GPIOA->CRL =0x94944111;
GPIOA->CRH =0x44444494; 
RCC->APB2ENR  |= 1<<12|1<<14;  // Enable clock for SPI1 USART1
SPI1->CR1 |= 0x33c;      // SSM 0x33c
...
void LCDBitmap(char my_array[]){
  ...
  for (i=0;i<(LCD_X*LCD_Y/8);i++){
    send_grdata(my_array[i]);
		
	}
}

void gotoXY(uint16_t x,uint16_t y) {
  send_cmd(0x80 | x);  // Column.
  send_cmd(0x40 | y);  // Row.  ?
};
...
void LCDCharacter(char character) {
  ...
  send_grdata(0x00); //Blank vertical line padding

  for (i = 0 ; i < 5 ; i++){
    send_grdata(ASCII[character - 0x21][i]);
